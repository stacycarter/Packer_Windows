...
## Automated Windows Updates script
## Script originally taken from the following github repository: https://github.com/joefitzgerald/packer-windows.  
## Several modifications were then made to the script.
## Changes made:  Removed the original openssh.ps1 references, added code to disable Malicious Software Removal Tool discovery, and added code to disable Automatic Updates.
## Calling script from Packer provisioner.
## This script creates a registry key on the machine and uses that to start this script after each reboot (since Windows Updates could take an unknown number of reboots).  
## Once there are no new Windows Updates found, this script removes the registry key and exits.
...
$NoAutoUpdateKey = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU"
$NoAutoUpdateEntry = "NoAutoUpdate"

$AUprop = (Get-ItemProperty $NoAutoUpdateKey).$NoAutoUpdateEntry
            if (-not $AUprop) 
                {
                LogWrite "NoAutoUpdate does not exist for Automatic Updates.  Creating it..."
                Set-ItemProperty -Path $NoAutoUpdateKey -Name $NoAutoUpdateEntry -Type DWord -Value 1 
                }
 
 ## End of setting Automatic Updates to Disabled
...
Function DisableUpdateMSRT {
	LogWrite "Disabling Malicious Software Removal Tool discovery..."
